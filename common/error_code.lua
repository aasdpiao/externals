...
SYSTEM_ERROR = {
	success                = add{code = 0x0000, desc = "成功"	    	},
	...
	not_check_validation   = add{code = 0x0014, desc = "未认证"			},
}
...
AUTH_ERROR = {
	...
	auth_failure       = add{code = 0x010a, desc = "验证失败"			},
	expired_version    = add{code = 0x010b, desc = "版本过期"			},
}

GAME_ERROR = {
	--不足
	gold_not_enough    			= add{code = 0x1001, desc = "金币不足"  },
	item_not_enough 			= add{code = 0x1002, desc = "物品不足" },
	--不能
	item_object_cant_upgrade 	= add{code = 0x2001, desc = "物品不能升级" },
	--不存在
	item_object_not_exist 		= add{code = 0x3001, desc = "物品不存在" },
	config_not_exist 			= add{code = 0x3002, desc = "配置不存在" },
}
